...
在旧版浏览器（主要是 Internet Explorer 9 及更早版本）中，恶意页面可以包含一个 <script> 标签指向受害者的 JSON API 端点。
...
这会导致浏览器的 JavaScript 引擎进入无限循环（如果通过 <script> 标签加载响应），从而防止数据被访问。
合法的 API 消费者（使用 fetch、XMLHttpRequest 或任何 HTTP 客户端）读取原始响应体，只需在解析前去除前缀即可
