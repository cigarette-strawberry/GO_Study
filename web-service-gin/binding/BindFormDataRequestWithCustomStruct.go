...
/*
	Gin 可以自动将表单数据绑定到嵌套结构体中。
	当你的数据模型由较小的结构体组成时——无论是嵌入字段、指针字段还是匿名内联结构体——Gin 会遍历结构体层次并将每个 form 标签映射到相应的查询参数或表单字段。

	这对于将复杂表单组织成可重用的子结构非常有用，而不是定义一个包含许多字段的扁平结构体。

	所有三种模式——嵌套结构体、嵌套结构体指针和匿名内联结构体——都使用相同的扁平查询参数进行绑定。
	Gin 不要求参数名称中有任何前缀或嵌套约定。
*/
